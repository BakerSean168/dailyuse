...
export class SqliteAuthIdentityRepository implements IAuthIdentityRepository {
  ...
  async findById(id: string): Promise<AuthIdentity | null> {
    const row = this.db.prepare(`SELECT * FROM auth_identities WHERE id = ?`).get(id) as
      | AuthIdentityRow
      | undefined;
    if (!row) return null;
  ...
  async existsByEmail(email: string): Promise<boolean> {
    const result = this.db
      .prepare(`SELECT COUNT(*) as cnt FROM auth_identifiers WHERE type = 'Email' AND value = ?`)
  ...
  async existsByPhone(phoneNumber: string): Promise<boolean> {
    const result = this.db
      .prepare(`SELECT COUNT(*) as cnt FROM auth_identifiers WHERE type = 'Phone' AND value = ?`)

 ...
 * Residual 188: lock bare findById as trusted-token bootstrap only; user-facing
 * session reads/mutations must use findByIdForIdentity.
 */
describe('auth session ownership surface', () => {
  ...
  const refreshToken = readFileSync(
    resolve(
      __dirname,
      '../../../../application/use-cases/commands/refresh-token.use-case.ts',
    ),
    'utf8',
  );
  ...
  it('bare findById remains only for trusted-token bootstrap; user paths re-own (residual 188)', () => {
    // Dual method kept intentionally: JWT/refresh load by session id from signed claims.
    expect(port).toContain('findById(id: AuthSessionId): Promise<AuthSession | null>;');
    expect(port).toContain(
      'findByIdForIdentity(identityId: IdentityId, id: AuthSessionId): Promise<AuthSession | null>;',
    );
    expect(prisma).toContain('async findById(id: string): Promise<AuthSession | null>');
    expect(prisma).toContain('async findByIdForIdentity(identityId: string, id: string)');
    expect(prisma).toContain('where: { id, identityId }');
    expect(powersync).toContain(
      'SELECT * FROM auth_sessions WHERE id = ? AND identity_id = ? LIMIT 1',
    ...
    // Trusted-token paths may bare-load by session id from the token.
    expect(refreshToken).toMatch(
      /sessionRepository\.findById\(AuthSessionId\.of\(tokenSessionId as string\)\)/,
    );
    expect(jwtStrategy).toMatch(
      /sessionRepository\.findById\(AuthSessionId\.of\(payload\.sessionId as string\)\)/,
    );

    // User-facing session get/revoke never authorize by bare session PK alone.
    expect(revoke).toContain('findByIdForIdentity(');
    expect(revoke).not.toMatch(/sessionRepository\.findById\(/);
    expect(revoke).toContain("error('NOT_FOUND', 'Session not found')");
    expect(revoke).not.toContain("error('FORBIDDEN'");

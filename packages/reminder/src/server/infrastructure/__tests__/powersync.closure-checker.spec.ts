...
describe('Reminder PowerSync closureChecker (Desktop local new-work gate)', () => {
  const identityId = 'IdentityId_desktop-close-window-1';
  ...
  it('blocks new work on query failure (fail-closed)', async () => {
    const brokenDb: IElectronDatabase = {
    ...
    const blocked = await createPowerSyncClosureChecker(brokenDb)(identityId);
    expect(blocked).toBe(true);
  });
});

/**
 * Electron IPC auth wrapper for the goal module.
 * 目标模块的 Electron IPC 鉴权包装器。
 *
 ...
 * Existing positional channels pass a `projectArgs` projector that composes the
 * raw positional args into the contract request shape (e.g.
 ...
 * `IpcResult`。既有 positional channel 通过 `projectArgs` projector 把原始
 * positional args 组合成 contract 请求形状（例如
 * `(args) => ({ params: { id: args[0] }, body: args[1] })`）。
 *
 ...
 * @param projectArgs - Optional projector from positional args to canonical input.

...
import type { IGoalStatisticsRepository, IGoalRepository } from '@/domain-server';
import { GoalStatisticsDomainService } from '@/domain-server';
import type {
  GoalServerDTO,
  GoalClientDTO,
  KeyResultServerDTO,
...
export class GoalStatisticsApplicationService {
  ...
  constructor(statisticsRepository: IGoalStatisticsRepository, goalRepository: IGoalRepository) {
  ...
  async getOrCreateStatistics(accountUuid: string): Promise<GoalStatisticsClientDTO> {
    ...
    if (!statistics) {
      // 使用聚合根的工厂方法创建
      const { GoalStatistics } = await import('@dailyuse/goal/domain-server');

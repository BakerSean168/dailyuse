  ...
  <Card class="h-full flex flex-col">
    <CardHeader class="flex flex-row items-center justify-between space-y-0 p-4">
      <div class="flex items-center">
        <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-green-100 text-green-600 dark:bg-green-900/30 mr-3">
          <ListChecks class="h-5 w-5" />
        </div>
        <div>
          <CardTitle class="text-lg font-bold">任务模块任务</CardTitle>
          <p class="text-xs text-muted-foreground">Task Module Tasks</p>
        </div>
      </div>
    ...
    <CardContent class="p-4 flex-1">
      ...
      <div v-else class="text-center py-8">
        ...
        <p class="text-sm text-muted-foreground">暂无任务模块任务</p>
      </div>
    </CardContent>
  </Card>
</template>

// Goal Module Components
export { default as AIGenerateKRButton } from './AIGenerateKRButton.vue';
export { default as AIKeyResultsSection } from './AIKeyResultsSection.vue';
export { default as ActivateFocusModeDialog } from './ActivateFocusModeDialog.vue';
export { default as FocusModeHistoryPanel } from './FocusModeHistoryPanel.vue';
export { default as FocusModeStatusBar } from './FocusModeStatusBar.vue';
export { default as GoalRecordCard } from './GoalRecordCard.vue';
export { default as KRPreviewList } from './KRPreviewList.vue';
export { default as ProgressBreakdownPanel } from './ProgressBreakdownPanel.vue';

// Cards
export { default as GoalCard } from './cards/GoalCard.vue';
export { default as GoalInfoShowCard } from './cards/GoalInfoShowCard.vue';
export { default as GoalRecordCardFromCards } from './cards/GoalRecordCard.vue';
export { default as GoalReviewListCard } from './cards/GoalReviewListCard.vue';
export { default as KeyResultCard } from './cards/KeyResultCard.vue';
export { default as MotivateCard } from './cards/MotivateCard.vue';

// Comparison
export { default as ComparisonStatsPanel } from './comparison/ComparisonStatsPanel.vue';
export { default as MultiGoalSelector } from './comparison/MultiGoalSelector.vue';

// DAG
export { default as ExportDialog } from './dag/ExportDialog.vue';
export { default as GoalDAGVisualization } from './dag/GoalDAGVisualization.vue';

// Demos
export { default as GoalCardDemo } from './demos/GoalCardDemo.vue';
export { default as GoalReviewCardDemo } from './demos/GoalReviewCardDemo.vue';

// Dialogs
export { default as GoalDialog } from './dialogs/GoalDialog.vue';
export { default as GoalFolderDialog } from './dialogs/GoalFolderDialog.vue';
export { default as GoalRecordDialog } from './dialogs/GoalRecordDialog.vue';
export { default as KeyResultDialog } from './dialogs/KeyResultDialog.vue';

// ECharts
export { default as GoalGanttChart } from './echarts/GoalGanttChart.vue';
export { default as GoalProgressChart } from './echarts/GoalProgressChart.vue';
export { default as KrCompletionChart } from './echarts/KrCompletionChart.vue';
export { default as KrProgressChart } from './echarts/KrProgressChart.vue';
export { default as KrWeightDistributionChart } from './echarts/KrWeightDistributionChart.vue';
export { default as PeriodBarChart } from './echarts/PeriodBarChart.vue';
export { default as ReviewProgressChart } from './echarts/ReviewProgressChart.vue';

// Rules
export { default as StatusRuleEditor } from './rules/StatusRuleEditor.vue';

// Template
export { default as TemplateBrowser } from './template/TemplateBrowser.vue';

// Timeline
export { default as GoalTimelineView } from './timeline/GoalTimelineView.vue';
export { default as TimelineControls } from './timeline/TimelineControls.vue';

// Weight Snapshot
export { default as WeightComparison } from './weight-snapshot/WeightComparison.vue';
export { default as WeightSnapshotList } from './weight-snapshot/WeightSnapshotList.vue';
export { default as WeightTrendChart } from './weight-snapshot/WeightTrendChart.vue';

// Weight
export { default as WeightSuggestionPanel } from './weight/WeightSuggestionPanel.vue';

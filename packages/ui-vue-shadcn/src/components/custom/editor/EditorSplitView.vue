<template>
  <div class="flex w-full h-full relative overflow-hidden">
    ...
    <div v-if="showEditor && showPreview" class="divider-container" @mousedown="startResize">
      ...
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-10 bg-border rounded-full"></div>
    </div>
...
.divider-container {
  @apply relative w-2 bg-muted/50 cursor-col-resize flex items-center justify-center transition-colors hover:bg-muted shrink-0;
}
</style>

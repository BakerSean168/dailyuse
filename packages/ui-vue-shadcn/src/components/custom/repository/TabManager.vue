<template>
  <div class="border-b border-border bg-background">
    <Tabs v-model="localActiveTab" class="w-full">
      <TabsList class="h-9 w-full justify-start rounded-none border-b-0 bg-transparent p-0">
        <TabsTrigger
          ...
          @click="emit('switch-tab', tab.uuid)"
        ...
        >
          ...
          <Button
            ...
            @click.stop="emit('close-tab', tab.uuid)"
          >
    ...
    <DropdownMenu v-model:open="contextMenu.show">
      ...
      <DropdownMenuContent>
        <DropdownMenuItem @click="emit('toggle-pin', contextMenu.tab?.uuid)">
          <component :is="contextMenu.tab?.isPinned ? PinOffIcon : PinIcon" class="mr-2 h-4 w-4" />
          {{ contextMenu.tab?.isPinned ? '取消固定' : '固定' }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem @click="emit('close-tab', contextMenu.tab?.uuid)">
          <component :is="XIcon" class="mr-2 h-4 w-4" />
          关闭
        </DropdownMenuItem>
        <DropdownMenuItem @click="emit('close-others', contextMenu.tab?.uuid)">
          <component :is="XIcon" class="mr-2 h-4 w-4" />
          关闭其他
        </DropdownMenuItem>
        <DropdownMenuItem @click="emit('close-right', contextMenu.tab?.uuid)">

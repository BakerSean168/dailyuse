  ...
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <div class="flex items-center gap-1 p-2 border-b border-border">
      <Button variant="ghost" size="icon-sm" @click="$emit('create-resource', undefined)">
        ...
        <TooltipProvider><Tooltip><TooltipTrigger as-child><span class="sr-only">新建笔记</span></TooltipTrigger><TooltipContent>新建笔记</TooltipContent></Tooltip></TooltipProvider>
      </Button>
      <Button variant="ghost" size="icon-sm" @click="$emit('create-folder', undefined)">
        <FolderPlus class="h-4 w-4" />
        <TooltipProvider><Tooltip><TooltipTrigger as-child><span class="sr-only">新建文件夹</span></TooltipTrigger><TooltipContent>新建文件夹</TooltipContent></Tooltip></TooltipProvider>
      </Button>
      <Button variant="ghost" size="icon-sm" @click="$emit('ai-generate-knowledge', undefined)">
        <Bot class="h-4 w-4" />
        <TooltipProvider><Tooltip><TooltipTrigger as-child><span class="sr-only">AI 生成知识文档</span></TooltipTrigger><TooltipContent>AI 生成知识文档</TooltipContent></Tooltip></TooltipProvider>
      </Button>
      <div class="flex-1" />
    ...
    <!-- Loading state -->
    <div v-else-if="loading && nodes.length === 0" class="flex flex-col items-center justify-center flex-1">
      <Loader2 class="h-6 w-6 animate-spin" />
    </div>

    <!-- Tree content -->
    <div v-else class="flex-1 overflow-y-auto p-1" @contextmenu.prevent="handleEmptyAreaContextMenu">
      <!-- Empty tree -->
      <div v-if="nodes.length === 0" class="flex flex-col items-center justify-center p-8 text-center">

...
<template>
  <div class="flex flex-col h-full bg-background">
    ...
    <div class="flex-1 overflow-y-auto p-2">
      <!-- Loading -->
      <div v-if="isLoading && nodes.length === 0" class="flex flex-col items-center justify-center h-full gap-4">
        <Loader2 class="w-8 h-8 animate-spin text-muted-foreground" />
        <span class="text-sm text-muted-foreground">加载文件树...</span>
      </div>

      <!-- Empty -->
      <div v-else-if="!isLoading && nodes.length === 0" class="flex flex-col items-center justify-center h-full gap-4 text-center px-4">
...
import { RefreshCw, FolderPlus, FilePlus, FolderOpen, Loader2, Plus, UnfoldVertical, Maximize2 } from 'lucide-vue-next';

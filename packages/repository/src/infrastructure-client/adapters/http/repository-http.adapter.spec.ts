...
describe('RepositoryHttpAdapter legacy database repository surface', () => {
  it('hard-fails removed Repository/Folder/Resource methods without calling HTTP', async () => {
    const httpClient = createHttpClient();
    const adapter = new RepositoryHttpAdapter(httpClient);
    ...
    await expect(
      adapter.createResource('repo-1', { name: 'x.md' } as never),
    ).resolves.toMatchObject({
      ok: false,
      error: { code: 'NOT_SUPPORTED' },
    });
    await expect(adapter.deleteResource('resource-1')).resolves.toMatchObject({
      ok: false,
      error: { code: 'NOT_SUPPORTED' },
    });
    await expect(adapter.getFileTree('repo-1')).resolves.toMatchObject({
      ok: false,
      error: { code: 'NOT_SUPPORTED' },
    });

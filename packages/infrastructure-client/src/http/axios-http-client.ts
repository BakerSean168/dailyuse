 ...
 *
 * 对接现有的 Hexagonal Architecture 适配层。
 * 所有现有的 HTTP Adapters（GoalHttpAdapter、TaskHttpAdapter 等）
...
import type { HttpClient, HttpRequestOptions } from '../shared/http-client.types';
 ...
 *
 * 专为现有 DI 体系（`configureWebDependencies(httpClient)`）设计。
 * 签名与 `HttpClient` 接口保持一致：`Promise<T>`（不返回 Result）。
 *
 * @example
 * ```ts
 * import { AxiosHttpClient } from '@dailyuse/infrastructure-client';
 *

 ...
 * Residual 1218: tip focused suite pointer refresh (Residual 1217 evidence tip 367/1584)
 ...
 * Residual 1215 (soft): prior tip refresh 366/1579 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1218');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('367 文件 / 1584 测试');
    expect(sec).toContain('Residual 1217');
    expect(sec).toContain('Residual 1218');

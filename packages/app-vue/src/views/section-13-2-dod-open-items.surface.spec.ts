 ...
 * Residual 1147: tip focused suite pointer refresh (Residual 1146 evidence tip 344/1488)
 ...
 * isPlainObject + toMillis + formatFileSize + clampPercentage + isRecord keep-boundaries (no force-merge).
 ...
 * Residual 1143 (soft): prior tip refresh 343/1484 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1147');
    expect(plan).toContain('残留一千一百四十七轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('344 文件 / 1488 测试');
    expect(sec).toContain('Residual 1146');
    expect(sec).toContain('Residual 1147');

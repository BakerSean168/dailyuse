 ...
 * Residual 1296: tip focused suite pointer refresh (Residual 1295 evidence tip 393/1716)
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1296');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('393 文件 / 1716 测试');
    expect(sec).toContain('Residual 1295');
    expect(sec).toContain('Residual 1296');
    ...
    expect(sec).toContain('formatHHmmParts dual-retired 不强制');

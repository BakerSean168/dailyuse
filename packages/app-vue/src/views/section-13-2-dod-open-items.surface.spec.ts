 ...
 * Residual 1151: tip focused suite pointer refresh (Residual 1150 evidence tip 345/1492)
 ...
 * isPlainObject + toMillis + formatFileSize + toKnowledgeNoteRef + clampPercentage + isRecord keep-boundaries (no force-merge).
 ...
 * Residual 1147 (soft): prior tip refresh 344/1488 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1151');
    expect(plan).toContain('残留一千一百五十一轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('345 文件 / 1492 测试');
    expect(sec).toContain('Residual 1150');
    expect(sec).toContain('Residual 1151');

 ...
 * Residual 1230: tip focused suite pointer refresh (Residual 1229 evidence tip 371/1606)
 ...
 * Residual 1227 (soft): prior tip refresh 370/1600 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1230');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('371 文件 / 1606 测试');
    expect(sec).toContain('Residual 1229');
    expect(sec).toContain('Residual 1230');

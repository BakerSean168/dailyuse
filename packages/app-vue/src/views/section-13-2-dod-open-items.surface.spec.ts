 ...
 * Residual 1224: tip focused suite pointer refresh (Residual 1223 evidence tip 369/1595)
 ...
 * Residual 1221 (soft): prior tip refresh 368/1589 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1224');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('369 文件 / 1595 测试');
    expect(sec).toContain('Residual 1223');
    expect(sec).toContain('Residual 1224');

 ...
 * Residual 1221: tip focused suite pointer refresh (Residual 1220 evidence tip 368/1589)
 ...
 * Residual 1218 (soft): prior tip refresh 367/1584 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1221');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('368 文件 / 1589 测试');
    expect(sec).toContain('Residual 1220');
    expect(sec).toContain('Residual 1221');

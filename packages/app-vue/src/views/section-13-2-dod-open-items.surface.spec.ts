 ...
 * Residual 1173: tip focused suite pointer refresh (Residual 1172 evidence tip 352/1520)
 ...
 * isPlainObject + toMillis + formatFileSize + toKnowledgeNoteRef + tokenize + toDashboardTaskInstanceRecord + toPrismaJson + contracts-isRecord + startOfDay + mapImportanceToTaskPriority + readString + clampPercentage + isRecord keep-boundaries (no force-merge).
 ...
 * Residual 1170 (soft): prior tip refresh 351/1516 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1173');
    expect(plan).toContain('残留一千一百七十三轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('352 文件 / 1520 测试');
    expect(sec).toContain('Residual 1172');
    expect(sec).toContain('Residual 1173');

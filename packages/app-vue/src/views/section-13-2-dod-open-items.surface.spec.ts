 ...
 * Residual 1239: tip focused suite pointer refresh (Residual 1238 evidence tip 374/1621)
 ...
 * Residual 1236 (soft): prior tip refresh 373/1616 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1239');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('374 文件 / 1621 测试');
    expect(sec).toContain('Residual 1238');
    expect(sec).toContain('Residual 1239');
    ...
    expect(sec).toContain('formatTime 不强制');

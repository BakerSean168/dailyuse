 ...
 * Residual 1170: tip focused suite pointer refresh (Residual 1169 evidence tip 351/1516)
 ...
 * isPlainObject + toMillis + formatFileSize + toKnowledgeNoteRef + tokenize + toDashboardTaskInstanceRecord + toPrismaJson + contracts-isRecord + startOfDay + mapImportanceToTaskPriority + clampPercentage + isRecord keep-boundaries (no force-merge).
 ...
 * Residual 1167 (soft): prior tip refresh 350/1512 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1170');
    expect(plan).toContain('残留一千一百七十轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('351 文件 / 1516 测试');
    expect(sec).toContain('Residual 1169');
    expect(sec).toContain('Residual 1170');

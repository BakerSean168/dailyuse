 ...
 * Residual 1305: tip focused suite pointer refresh (Residual 1304 evidence tip 396/1731)
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1305');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('396 文件 / 1731 测试');
    expect(sec).toContain('Residual 1304');
    expect(sec).toContain('Residual 1305');
    ...
    expect(sec).toContain('formatCalendarEventTimeRange→formatLocalHHmm dual-retired 不强制');

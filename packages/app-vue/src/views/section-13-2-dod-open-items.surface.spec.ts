 ...
 * Residual 1212: tip focused suite pointer refresh (Residual 1211 evidence tip 365/1574)
 ...
 * Residual 1209 (soft): prior tip refresh 364/1570 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1212');
    expect(plan).toContain('残留一千二百一十二轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('365 文件 / 1574 测试');
    expect(sec).toContain('Residual 1211');
    expect(sec).toContain('Residual 1212');

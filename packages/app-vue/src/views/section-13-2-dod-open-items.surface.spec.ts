 ...
 * Residual 1215: tip focused suite pointer refresh (Residual 1214 evidence tip 366/1579)
 ...
 * Residual 1212 (soft): prior tip refresh 365/1574 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1215');
    expect(plan).toContain('残留一千二百一十五轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('366 文件 / 1579 测试');
    expect(sec).toContain('Residual 1214');
    expect(sec).toContain('Residual 1215');

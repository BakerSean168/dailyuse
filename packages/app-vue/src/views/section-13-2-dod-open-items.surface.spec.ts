 ...
 * Residual 1236: tip focused suite pointer refresh (Residual 1235 evidence tip 373/1616)
 ...
 * Residual 1233 (soft): prior tip refresh 372/1611 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1236');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('373 文件 / 1616 测试');
    expect(sec).toContain('Residual 1235');
    expect(sec).toContain('Residual 1236');

 ...
 * Residual 1233: tip focused suite pointer refresh (Residual 1232 evidence tip 372/1611)
 ...
 * Residual 1230 (soft): prior tip refresh 371/1606 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1233');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('372 文件 / 1611 测试');
    expect(sec).toContain('Residual 1232');
    expect(sec).toContain('Residual 1233');

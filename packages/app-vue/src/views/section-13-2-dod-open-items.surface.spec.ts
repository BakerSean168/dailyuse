 ...
 * Residual 1197: tip focused suite pointer refresh (Residual 1196 evidence tip 360/1552)
 ...
 * Residual 1194 (soft): prior tip refresh 359/1548 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1197');
    expect(plan).toContain('残留一千一百九十七轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('360 文件 / 1552 测试');
    expect(sec).toContain('Residual 1196');
    expect(sec).toContain('Residual 1197');

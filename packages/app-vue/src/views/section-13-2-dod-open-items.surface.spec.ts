 ...
 * Residual 1272: tip focused suite pointer refresh (Residual 1271 evidence tip 385/1676)
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1272');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('385 文件 / 1676 测试');
    expect(sec).toContain('Residual 1271');
    expect(sec).toContain('Residual 1272');
    ...
    expect(sec).toContain('formatCalendarEventTimeRange dual-retired 不强制');

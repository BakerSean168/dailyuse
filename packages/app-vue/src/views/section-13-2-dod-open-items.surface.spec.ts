 ...
 * Residual 1129: tip focused suite pointer refresh (Residual 1128 evidence tip 339/1468)
 ...
 * asNonEmptyString dual-retired + toDate/toDateString + extractErrorMessage dual-retired + clampPercentage + isRecord keep-boundaries (no force-merge).
 ...
 * Residual 1125 (soft): prior tip refresh 338/1464 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1129');
    expect(plan).toContain('残留一千一百二十九轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('339 文件 / 1468 测试');
    expect(sec).toContain('Residual 1128');
    expect(sec).toContain('Residual 1129');
    ...
    expect(sec).not.toContain('focused evidence suite tip（Residual 1124）：**338 文件 / 1464 测试**');

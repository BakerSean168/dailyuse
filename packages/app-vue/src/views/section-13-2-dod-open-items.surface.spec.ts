 ...
 * Residual 1209: tip focused suite pointer refresh (Residual 1208 evidence tip 364/1570)
 ...
 * Residual 1206 (soft): prior tip refresh 363/1566 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1209');
    expect(plan).toContain('残留一千二百零九轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('364 文件 / 1570 测试');
    expect(sec).toContain('Residual 1208');
    expect(sec).toContain('Residual 1209');

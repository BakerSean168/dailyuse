 ...
 * Residual 1206: tip focused suite pointer refresh (Residual 1205 evidence tip 363/1566)
 ...
 * Residual 1203 (soft): prior tip refresh 362/1561 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1206');
    expect(plan).toContain('残留一千二百零六轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('363 文件 / 1566 测试');
    expect(sec).toContain('Residual 1205');
    expect(sec).toContain('Residual 1206');

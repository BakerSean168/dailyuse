 ...
 * Residual 1311: tip focused suite pointer refresh (Residual 1310 evidence tip 398/1741)
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1311');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('398 文件 / 1741 测试');
    expect(sec).toContain('Residual 1310');
    expect(sec).toContain('Residual 1311');
    ...
    expect(sec).toContain('dashboard formatTime absolute→formatLocalHHmm dual-retired 不强制');

 ...
 * Residual 1188: tip focused suite pointer refresh (Residual 1187 evidence tip 357/1540)
 ...
 * Residual 1185 (soft): prior tip refresh 356/1536 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1188');
    expect(plan).toContain('残留一千一百八十八轮');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('357 文件 / 1540 测试');
    expect(sec).toContain('Residual 1187');
    expect(sec).toContain('Residual 1188');

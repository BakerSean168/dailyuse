 ...
 * Residual 1227: tip focused suite pointer refresh (Residual 1226 evidence tip 370/1600)
 ...
 * Residual 1224 (soft): prior tip refresh 369/1595 still in history notes only.
...
describe('§13.2 DoD open items honest audit (residual 891)', () => {
  ...
  it('keeps exactly three unchecked §13.2 items with partial/external-block labels', () => {
    ...
    expect(plan).toContain('Residual 1227');
  ...
  it('records tip focused suite evidence without claiming full PR gate completion', () => {
    const sec = section132();
    expect(sec).toContain('370 文件 / 1600 测试');
    expect(sec).toContain('Residual 1226');
    expect(sec).toContain('Residual 1227');

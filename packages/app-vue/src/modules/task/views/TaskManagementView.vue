<template>
  <section class="flex h-full min-h-0 flex-col overflow-hidden bg-background" data-testid="task-management-view">
    <header class="flex min-h-14 items-center gap-3 border-b px-4 py-2">
      <div>
      ...
      <div class="relative ml-auto w-56">
        ...
        <Input v-model="searchQuery" class="h-8 pl-8" :placeholder="t('common.search')" />
      </div>
    ...
    <div v-else class="min-h-0 flex-1 overflow-auto p-4">
      ...
      <div v-else class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="template in filteredTemplates" :key="template.id" class="rounded-lg border bg-card p-4 shadow-sm">
          ...
          <div class="mt-4 flex justify-end gap-1 border-t pt-3">
            ...
            <Button variant="ghost" size="sm" class="text-destructive" @click="remove(template.id)">{{ t('common.delete') }}</Button>
          </div>
        </article>
      </div>
    </div>
...
import { Badge, Button, Input } from '@memoflow/ui-vue-shadcn';
...
async function archive(id:string){if(await archiveTemplateSafe(id))await refetch();} async function remove(id:string){if(await deleteTemplateSafe(id))await refetch();}
</script>

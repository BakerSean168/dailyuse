...
import {
  logEditorIssue,
  summarizeResourceForDebug,
} from '../../../shared/utils/editor-issue-debug';
import {
...
export function useRepositoryResources(deps: ResourceDependencies) {
  ...
  async function fetchResources(): Promise<void> {
    ...
    const result = await deps.executeOperation(
      () => deps.service.listResources(repositoryId),
      '加载资源失败',
    );
    if (result.ok) {
  ...
  async function createResource(data: {
    ...
    try {
      const result = await deps.executeOperation(
        () => deps.service.createResource(repositoryId, { ...data }),
        '创建资源失败',
      );
  ...
  async function deleteResource(resourceId: string): Promise<boolean> {
    ...
    try {
      const result = await deps.executeOperation(
        () => deps.service.deleteResource(resourceId),
        '删除资源失败',
      );
      if (result.ok) {
  ...
  async function getResourceById(resourceId: string): Promise<ResourceClientDTO | null> {
    ...
    try {
      const result = await deps.executeOperation(
        () => deps.service.getResource!(resourceId),
        '加载资源失败',
      );
      if (result.ok && result.data) {
  ...
  async function readResourceAsDataUrl(resource: ResourceClientDTO): Promise<string> {
    ...
    return normalized.startsWith('data:')
      ? normalized
      : `data:${mimeType};base64,${normalized.replace(/\s+/g, '')}`;
  }
  ...
  async function updateResourceMetadata(
    ...
    try {
      const result = await deps.executeOperation(
        () => deps.service.updateResource(resourceId, { metadata }),
        '更新资源元数据失败',
      );
      if (result.ok && result.data) {
        deps.updateResourceInStore(result.data);
  ...
  async function createMarkdownNote(
    ...
    const noteName = ensureUniqueNoteName(
      requestedName,
      deps.getResources().filter((r) => r.folderId === (folderId ?? null)),
    );
    return createResource({
      name: noteName,
      type: 'File',
      mimeType: 'text/markdown',
      content: initialContent ?? buildInitialMarkdownContent(noteName),
      folderId,
    });
  }
  ...
  async function renameResource(
    ...
    const result = await deps.executeOperation(
      () => deps.service.renameResource!(resourceId, trimmedName),
      '重命名资源失败',
    );
    if (result.ok && result.data) {
      deps.updateResourceInStore(result.data);

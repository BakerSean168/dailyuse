  ...
  <Card class="h-full flex flex-col">
    <CardHeader class="flex flex-row items-center justify-between space-y-0 p-4">
      <div class="flex items-center">
        <div
          class="flex h-10 w-10 items-center justify-center rounded-lg bg-info/15 text-info mr-3"
        >
          <Bell class="h-5 w-5" />
        </div>
        <div>
          <CardTitle class="text-lg font-bold">{{ t('schedule.reminderCard.title') }}</CardTitle>
          <p class="text-xs text-muted-foreground">{{ t('schedule.reminderCard.subtitle') }}</p>
        </div>
      </div>
    ...
    <CardContent class="p-4 flex-1">
      ...
      <div v-if="isLoading" class="flex justify-center items-center py-8">
        <Loader2 class="h-12 w-12 animate-spin text-info" />
      </div>
      ...
      <div v-else class="text-center py-8">
        ...
        <p class="text-sm text-muted-foreground">{{ t('schedule.reminderCard.emptyTitle') }}</p>
      </div>
    </CardContent>
  </Card>
</template>

...
describe('useAITaskWorkflow', () => {
  ...
  it('projects clarification, review, and recovery stages without leaking raw failure messages', async () => {
    const { vm } = setup(
      run({
        status: 'suspended',
  ...
  it('deep-links only completed runs with a taskTemplateId', async () => {
    ...
    await vm.confirmTaskAgentRun();
    expect(options.openCreatedTask).toHaveBeenCalledWith('template-1');
  });

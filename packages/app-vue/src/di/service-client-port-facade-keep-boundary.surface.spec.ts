...
describe('DI I*Service ClientPort facade keep-boundary (residual 927)', () => {
  ...
  it('imports ClientPort types from package client entrypoints only once each', () => {
    expect(types).toContain("from '@dailyuse/authentication/client'");
    expect(types).toContain("from '@dailyuse/goal/client'");
    expect(types).toContain("from '@dailyuse/ai/client'");
    expect(types).toContain("from '@dailyuse/repository/client'");

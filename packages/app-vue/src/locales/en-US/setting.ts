export default {
  title: 'App Settings',
  userSettings: 'User Settings',
  groups: {
    appearance: 'Appearance & Language',
    repository: 'Knowledge Repository',
    ai: 'AI',
    notifications: 'Notifications & Reminders',
    account: 'Account & Privacy',
    data: 'Data',
    advanced: 'Advanced',
  },
  knowledgeRepository: {
    localTitle: 'Local Obsidian Vault',
    localDescription:
      'Desktop reads your local Vault directly. Notes stay local until you connect GitHub.',
    localNotSelected: 'No Vault selected',
    localNotSelectedHint: 'Choose an existing Obsidian Vault as your local knowledge base.',
    localSelect: 'Select Vault',
    localChange: 'Change Vault',
    localDetach: 'Detach',
    localDetachTitle: 'Detach the local Vault?',
    localDetachDescription:
      'This only removes the MemoFlow binding. No local files will be deleted or moved.',
    localDesktopOnly: 'Local Vault selection and management are available in Desktop only.',
    localSelectFailed: 'Failed to select the local Vault',
    localDetachFailed: 'Failed to detach the local Vault',
    githubTitle: 'GitHub Knowledge Repository',
    githubDescription:
      'Authorize a private repository through a dedicated GitHub App. This is fully separate from GitHub login.',
    permissionTitle: 'Least-privilege repository access',
    permissionDescription:
      'Only repositories explicitly selected during installation are visible. Desktop credentials are short-lived and restricted to one repository.',
    chooseTitle: 'Choose a knowledge repository',
    chooseDescription:
      'Only private, active repositories where you have administrator permission can be connected.',
    private: 'Private',
    public: 'Public',
    connect: 'Connect',
    disconnect: 'Disconnect',
    connectAnother: 'Connect another repository',
    startConnect: 'Connect GitHub Repository',
    createPrivate: 'Create private repository',
    createOpenFailed: 'Unable to open GitHub repository creation',
    notConnected: 'No GitHub knowledge repository connected',
    guestCloudBlocked:
      'Guest mode is local-only. Sign in with an account before connecting a GitHub knowledge repository.',
    offlineCloudBlocked:
      'Sign in online before connecting a GitHub knowledge repository. Offline and guest profiles cannot expand cloud authorization.',
    desktopConnectHint:
      'Authorization opens in your system browser. Return to Desktop and refresh after connecting.',
    webConnectHint:
      'After installing the GitHub App, you will return here to choose a private repository.',
    defaultBranch: 'Default branch: {branch}',
    lastSyncedCommit: 'Last synchronized commit: {sha}',
    errorTitle: 'Knowledge repository action failed',
    loadFailed: 'Failed to load knowledge repository connections',
    startFailed: 'Unable to start GitHub App installation',
    completeFailed: 'GitHub App installation validation failed. Start the connection again.',
    connectFailed: 'Failed to connect the GitHub knowledge repository',
    disconnectFailed: 'Failed to disconnect the GitHub knowledge repository',
    disconnectTitle: 'Disconnect the GitHub knowledge repository?',
    disconnectDescription:
      'Remote sync for {repository} will stop. Choose whether MemoFlow should retain or permanently delete its server-derived data.',
    purgeCloudData: 'Delete MemoFlow cloud projections and AI index',
    purgeCloudDataDescription:
      'Permanently deletes projected notes, attachment metadata and cache, webhook history, write ledger, and RAG index from MemoFlow.',
    purgeLocalAndGithubPreserved:
      'Your local Vault, local Git history, and GitHub repository are preserved. This cloud-data deletion cannot be undone.',
    retainCloudDataDescription:
      'MemoFlow will retain the rebuildable cloud projection so reconnecting can resume without a full rebuild. Your local Vault and GitHub repository are preserved.',
    reconciliation: {
      preview: 'Check first sync',
      previewFailed: 'First-sync preflight failed',
      execute: 'Run first sync',
      executeTitle: 'Run the first Git reconciliation?',
      executeDescription:
        '{action}\n\nMemoFlow will revalidate both sides and use a short-lived repository credential. It will never force-push.',
      executeFailed: 'First Git reconciliation failed',
      completed: 'First sync completed at GitHub HEAD {sha}',
      action: {
        InitializeRemoteFromLocal:
          'The local Vault has content and GitHub is empty. It is safe to initialize and push.',
        CloneRemoteIntoLocal:
          'The local Vault is empty and GitHub has content. It is safe to clone into this empty directory.',
        InitializeBoth: 'Both sides are empty. MemoFlow can create the scaffold and first commit.',
        ManualResolutionRequired:
          'Both sides contain content. Automatic overwrite is blocked; review the difference and import, use another repository, or cancel.',
      },
    },
    sync: {
      execute: 'Sync now',
      retry: 'Recheck sync',
      failed: 'Knowledge repository synchronization failed',
      conflictTitle: 'Git conflict needs attention',
      conflictDescription:
        'Both local and remote changes were preserved. Resolve the listed files and complete or abort the rebase in an external Git tool, then recheck sync.',
      conflictLocalHead: 'Local HEAD: {sha}',
      conflictRemoteHead: 'Remote HEAD: {sha}',
      openInObsidian: 'Open conflict in Obsidian',
      openInObsidianFailed: 'Failed to open the conflict in Obsidian',
      pendingTitle: 'Local commit saved; upload pending',
      pendingDescription:
        'Your Vault changes are safe in local commit {sha}. Recheck sync when GitHub is available.',
      outcome: {
        UpToDate: 'Already up to date at {sha}',
        Pushed: 'Local changes pushed to GitHub at {sha}',
        Pulled: 'Remote changes applied locally at {sha}',
        RebasedAndPushed: 'Local changes rebased and pushed at {sha}',
      },
    },
    lifecycle: {
      GITHUB_INSTALLATION_NOT_FOUND:
        'The GitHub App installation was removed. Reconnect the repository to resume synchronization.',
      GITHUB_INSTALLATION_SUSPENDED:
        'The GitHub App installation is suspended. Restore it on GitHub before synchronizing.',
      GITHUB_CONTENTS_PERMISSION_REQUIRED:
        'GitHub Contents write permission is missing. Update the GitHub App installation.',
      GITHUB_REPOSITORY_ACCESS_LOST:
        'The repository was deleted or removed from this GitHub App installation.',
      GITHUB_REPOSITORY_PUBLIC:
        'This repository is public. Synchronization is paused to protect knowledge privacy.',
      GITHUB_REPOSITORY_ARCHIVED: 'This repository is archived. Unarchive it before synchronizing.',
      GITHUB_REPOSITORY_DISABLED:
        'This repository is disabled on GitHub. Synchronization is paused.',
      GITHUB_REPOSITORY_ADMIN_REQUIRED:
        'Repository administrator permission is required to continue synchronization.',
      GITHUB_DEFAULT_BRANCH_CHANGED:
        'The GitHub default branch changed. Reconcile the repository before synchronizing again.',
      GITHUB_LIFECYCLE_CHECK_UNAVAILABLE:
        'GitHub status could not be checked. Local Vault access remains available.',
      unknown: 'The GitHub repository connection requires attention.',
    },
    status: {
      PendingInstall: 'Waiting for installation',
      Active: 'Connected',
      Suspended: 'Suspended',
      Revoked: 'Revoked',
      Error: 'Error',
    },
  },
  appearance: {
    title: 'Appearance Settings',
    theme: 'Theme',
    themePlaceholder: 'Select theme',
    themeDescription: 'Choose your preferred theme color scheme',
    fontSize: 'Font Size',
    fontSizePlaceholder: 'Select font size',
    accentColor: 'Accent Color',
    accentColorDescription: 'Customize the accent color of the theme',
    compactMode: 'Compact Mode',
    compactModeDescription: 'Reduce spacing between elements to show more content',
    fontFamily: 'Font',
    fontFamilyPlaceholder: 'Select font',
    themeLight: 'Standard Light',
    themeDark: 'Standard Dark',
    themeAuto: 'System',
    themeDarkBlue: 'Dark Blue',
    themeWarmPaper: 'Warm Paper (Light)',
    themeLightBlue: 'Light Blue',
    themeBlueGreen: 'Blue Green (Dark)',
    fontSmall: 'Small',
    fontMedium: 'Medium',
    fontLarge: 'Large',
    fontDefault: 'System Default',
    fontMSYH: 'Microsoft YaHei',
    fontPingFang: 'PingFang SC',
  },
  locale: {
    title: 'Region Settings',
    language: 'Language',
    languagePlaceholder: 'Select language',
    timezone: 'Timezone',
    timezonePlaceholder: 'Select timezone',
    dateFormat: 'Date Format',
    dateFormatPlaceholder: 'Select date format',
    timeFormat: 'Time Format',
    timeFormatPlaceholder: 'Select time format',
    weekStartsOn: 'Week Starts On',
    weekStartsOnPlaceholder: 'Select day',
    currency: 'Currency',
    currencyPlaceholder: 'Select currency',
    tzBeijing: 'Beijing Time (UTC+8)',
    tzTokyo: 'Tokyo Time (UTC+9)',
    tzNewYork: 'New York Time (UTC-5)',
    tzLondon: 'London Time (UTC+0)',
    tzSydney: 'Sydney Time (UTC+11)',
    dateISO: '2025-10-31 (YYYY-MM-DD)',
    dateDMY: '31/10/2025 (DD/MM/YYYY)',
    dateMDY: '10/31/2025 (MM/DD/YYYY)',
    dateChinese: 'Oct 31, 2025',
    time24h: '24-hour (23:59)',
    time12h: '12-hour (11:59 PM)',
    weekSunday: 'Sunday',
    weekMonday: 'Monday',
    weekSaturday: 'Saturday',
    currencyCNY: 'Chinese Yuan (CNY ¥)',
    currencyUSD: 'US Dollar (USD $)',
    currencyEUR: 'Euro (EUR €)',
    currencyJPY: 'Japanese Yen (JPY ¥)',
    currencyGBP: 'British Pound (GBP £)',
  },
  workflow: {
    title: 'Workflow Settings',
    autoSave: 'Auto Save',
    autoSaveDescription: 'Automatically save changes when editing',
    autoSaveInterval: 'Auto Save Interval (seconds)',
    confirmBeforeDelete: 'Confirm Before Delete',
    confirmBeforeDeleteDescription: 'Show confirmation dialog before deleting items',
    defaultGoalView: 'Default Goal View',
    defaultScheduleView: 'Default Schedule View',
    defaultTaskView: 'Default Task View',
    viewPlaceholder: 'Select view',
    listView: 'List View',
    kanbanView: 'Kanban View',
    ganttView: 'Gantt Chart',
    treeView: 'Tree View',
    dayView: 'Day View',
    weekView: 'Week View',
    monthView: 'Month View',
    calendarView: 'Calendar View',
    matrixView: 'Matrix View',
  },
  repository: {
    imageEmbed: 'Image Embed',
    embedMode: 'Embed Mode',
    embedModeDescription: 'Default handling when inserting images into notes',
    embedModePlaceholder: 'Select mode',
    autoEmbedThreshold: 'Auto Embed Threshold',
    autoEmbedThresholdDescription: 'Images smaller than this size will be auto-converted to Base64',
    imageCompression: 'Image Compression',
    enableCompression: 'Enable Image Compression',
    enableCompressionDescription:
      'Automatically compress images when embedding or uploading to reduce file size',
    compressionQuality: 'Compression Quality',
    compressionQualityDescription: 'Lower values produce smaller files but reduce image quality',
    autoConvertWebP: 'Auto Convert to WebP',
    autoConvertWebPDescription:
      'WebP format is typically smaller than PNG/JPEG and supports transparency',
    maxImageWidth: 'Max Image Width',
    maxImageWidthDescription: 'Images wider than this will be automatically scaled down',
    maxImageWidthPlaceholder: 'Select width',
    resourceManagement: 'Resource Management',
    resourceStorageInfo:
      'Resources (images, audio, video, etc.) are stored in the {dir} directory of the repository',
    defaultViewMode: 'Default View Mode',
    defaultViewModeDescription: 'Default display mode when opening a repository',
    notesView: 'Notes',
    resourcesView: 'Resources',
    notesViewLabel: 'Notes view',
    resourcesViewLabel: 'Resources view',
    embedModeLink: 'Link Reference (Recommended)',
    embedModeBase64: 'Base64 Embed',
    embedModeAuto: 'Auto (Embed small images)',
    maxWidth800: '800px (Small)',
    maxWidth1280: '1280px (Medium)',
    maxWidth1920: '1920px (Large)',
    maxWidth2560: '2560px (Extra Large)',
    maxWidthNone: 'No limit',
  },
  ai: {
    title: 'AI Assistant',
    description: 'Configure AI provider connections and the default model for AI workflows.',
    quickProviderSectionTitle: 'Quick Provider Setup',
    quickProviderSectionDescription:
      'Configure mainstream providers with just an API key. Models are discovered automatically when supported.',
    quickProviderConfigured: 'Configured',
    quickProviderConnect: 'Connect',
    quickProviderConnecting: 'Connecting...',
    quickProviderUpdate: 'Update Key',
    quickProviderUpdating: 'Updating...',
    quickProviderNotConfigured: 'Not configured yet.',
    quickProviderConfiguredSummary: 'Default model: {model} · {count} discovered model(s).',
    quickProviderCurrentDefault: 'This provider is currently the default for AI workflows.',
    quickProviderDefaultHint: 'Use this provider as the default after connect or update.',
    quickProviderApiKeyPlaceholder: '{provider} API key',
    getApiKey: 'Get API key',
    providerSectionTitle: 'Provider Configuration',
    providerSectionDescription:
      'Review configured providers and choose which one is the default for AI workflows.',
    refreshProviders: 'Refresh list',
    refreshModels: 'Refresh models',
    refreshingModels: 'Refreshing...',
    defaultProvider: 'Default',
    inactiveProvider: 'Inactive',
    setDefaultProvider: 'Set default',
    providerModelsSummary: '{count} cached model(s)',
    manageAdvancedConfig: 'Advanced config',
    loadIntoForm: 'Load into form',
    deleteProvider: 'Delete',
    noProviders: 'No AI providers yet. Create one from the form on the right.',
    advancedProviderSectionTitle: 'Advanced Model Configuration',
    advancedProviderSectionDescription:
      'Use the existing manual flow for custom base URLs, hand-picked models, or provider debugging.',
    providerFormTitle: 'Edit Provider',
    providerFormDescription: 'Supports any endpoint compatible with OpenAI chat completions.',
    providerNamePlaceholder: 'Provider name',
    providerBaseUrlPlaceholder: 'Base URL, for example https://api.openai.com/v1',
    providerModelPlaceholder: 'Model, for example gpt-4o-mini',
    providerApiKeyPlaceholder: 'API key',
    markAsDefault: 'Use as default provider',
    markAsDefaultDescription: 'Apply this provider as the default after create or update.',
    createProvider: 'Create provider',
    updateProvider: 'Update provider',
    testProvider: 'Test connection',
    testingProvider: 'Testing...',
    resetProviderForm: 'Clear form',
    providerCreated: 'Provider created',
    providerUpdated: 'Provider updated',
    providerDeleted: 'Provider deleted',
    providerDefaultUpdated: 'Default provider updated',
    providerModelsRefreshed: 'Provider models refreshed',
    providerModelsRefreshFailed: 'Failed to refresh provider models',
    providerActionFailed: 'Provider action failed',
    providerTestPassed: 'Connection test passed',
    providerTestFailed: 'Connection test failed',
  },
  privacy: {
    title: 'Privacy Settings',
    profileVisibility: 'Profile Visibility',
    profileVisibilityPlaceholder: 'Select visibility',
    showOnlineStatus: 'Show Online Status',
    showOnlineStatusDescription: 'Let other users see your online status',
    searchPermissions: 'Search Permissions',
    allowSearchByEmail: 'Allow search by email',
    allowSearchByPhone: 'Allow search by phone number',
    searchPermissionsDescription:
      'Control whether other users can find you by your contact information',
    dataSharing: 'Data Sharing',
    shareUsageData: 'Share Usage Data',
    shareUsageDataDescription: 'Help us improve the product. Your data will be anonymized',
    dataManagement: 'Data Management',
    exportData: 'Export Data',
    clearCache: 'Clear Cache',
    deleteAccount: 'Delete Account',
    privacyPolicyNotice: 'Your privacy is important to us. View our ',
    privacyPolicyLink: 'Privacy Policy',
    privacyPolicyNoticeEnd: ' to learn more about how we protect your data.',
    visibilityPublic: 'Public',
    visibilityPublicDesc: 'Everyone can view your profile',
    visibilityFriends: 'Friends',
    visibilityFriendsDesc: 'Only your friends can view',
    visibilityPrivate: 'Private',
    visibilityPrivateDesc: 'Only you can view',
  },
  shortcuts: {
    title: 'Keyboard Shortcuts',
    resetAll: 'Reset All',
    searchPlaceholder: 'Search shortcuts',
    pressKey: 'Press a key...',
  },
  notifications: {
    title: 'Notification preferences',
    description:
      'Set global delivery defaults, then override only the workflows that need different behavior.',
    desktopPresentationTitle: 'Desktop presentation',
    desktopPresentationDescription:
      'Choose how the Desktop host presents alerts that its runtime has already accepted for delivery.',
    useCustomNotification: 'Use MemoFlow notification windows',
    useCustomNotificationDescription:
      "When enabled, Desktop uses MemoFlow's compact notification window instead of the operating system's default presentation.",
    globalChannelsTitle: 'Global channels',
    globalChannelsDescription:
      'These defaults apply unless a workflow below explicitly overrides them.',
    globalInAppDescription:
      'Keep Notification Center facts and unread state available inside MemoFlow.',
    desktopCapabilityDescription:
      'Desktop delivery exists only when the active host provides the native Desktop capability. Availability and permission are owned by that device runtime.',
    systemManaged: 'System managed',
    unavailableChannelsDescription:
      'Email, mobile push, SMS, and webhooks are not shown as switches until a production delivery adapter is actually available.',
    workflowGroupsTitle: 'Workflow overrides',
    workflowGroupsDescription:
      'Leave a workflow on Follow global unless it must always be enabled or disabled for in-app delivery.',
    workflowModeLabel: 'In-app delivery preference',
    criticalReadOnly: 'Critical · read only',
    alwaysOn: 'Always on',
    criticalChannels: 'In-app + Desktop',
    criticalExplanation:
      'Explicit critical workflows such as account-security alerts cannot be disabled. This is a narrow allowlist, not a generic priority bypass.',
    modes: {
      inherit: 'Follow global',
      enabled: 'Always on',
      disabled: 'Off',
    },
    effectiveEnabled: 'Effective: on',
    effectiveDisabled: 'Effective: off',
    runtimePolicyTitle: 'Runtime policy',
    runtimePolicyDescription:
      'DND and rate limits are evaluated after workflow preferences; Notification facts remain visible even when a delivery attempt is deferred or suppressed.',
    dndTitle: 'Do Not Disturb',
    rateLimitTitle: 'Rate limit',
    policyOff: 'Not enabled',
    dndEnabledSummary: 'Active from {start} to {end} on the configured days.',
    rateLimitEnabledSummary: 'Up to {hour} per hour and {day} per day.',
    runtimePolicyOrder:
      'Critical account-security channels use the explicit read-only bypass. Other channels follow workflow preference → DND → rate-limit policy.',
    loading: 'Loading notification preferences…',
    retry: 'Retry',
    loadPreferencesFailed: 'Failed to load notification preferences',
    updatePreferencesFailed: 'Failed to update notification preferences',
    channels: {
      inApp: 'In-app notifications',
      desktop: 'Desktop alerts',
    },
    groups: {
      task: 'Tasks',
      goal: 'Goals',
      schedule: 'Schedule',
      reminder: 'Routines',
      account: 'Account',
      system: 'System',
    },
    workflows: {
      task: {
        general: {
          title: 'Task activity',
          description: 'Due tasks, occurrence changes, and other task-owned notices.',
        },
      },
      goal: {
        general: {
          title: 'Goal activity',
          description: 'Goal progress, review, and lifecycle notices.',
        },
      },
      schedule: {
        general: {
          title: 'Schedule activity',
          description: 'Planner and calendar notices owned by Schedule.',
        },
      },
      reminder: {
        general: {
          title: 'Routine activity',
          description: 'Routine occurrences and intervention notices.',
        },
      },
      account: {
        general: {
          title: 'Account activity',
          description: 'Normal profile and account lifecycle notices.',
        },
      },
      system: {
        general: {
          title: 'System activity',
          description: 'General MemoFlow service notices.',
        },
        accountSecurity: {
          title: 'Account security',
          description:
            'Sign-in, credential, and account-protection events that must remain visible.',
        },
      },
    },
  },
  experimental: {
    title: 'Experimental Features',
    warning: 'Warning',
    warningDescription:
      'Experimental features may be unstable and could be changed or removed in future versions. Enabling these features means you accept the potential risks.',
    enableExperimental: 'Enable Experimental Features',
    enableExperimentalDescription: 'Allow access to features still under development',
    availableFeatures: 'Available Experimental Features',
    noFeatures: 'No experimental features available',
    saveChanges: 'Save Changes',
    reset: 'Reset',
    new: 'New',
    aiAssistant: 'AI Assistant',
    aiAssistantDesc: 'AI-powered smart task suggestions and automation assistant',
    voiceInput: 'Voice Input',
    voiceInputDesc: 'Create tasks and notes using voice input',
    collaboration: 'Collaboration Mode',
    collaborationDesc: 'Real-time collaborative editing and shared workspaces',
    advancedAnalytics: 'Advanced Analytics',
    advancedAnalyticsDesc: 'In-depth productivity analytics and visual reports',
  },
  advanced: {
    title: 'Advanced Actions',
    exportSettings: 'Export Settings',
    exportJSON: 'Export as JSON',
    exportCSV: 'Export as CSV',
    importSettings: 'Import Settings',
    exportPortableData: 'Export Importable Data',
    exportingPortableData: 'Exporting...',
    importPortableData: 'Import Data File',
    importingPortableData: 'Importing...',
    portableDataDescription:
      'Creates an importable JSON backup of MemoFlow business data. It does not include local Vault files, GitHub authorization, GitHub-derived projections or attachment cache, or the RAG index. Export or clone the GitHub repository itself for the authoritative knowledge files.',
    exportServerDataDisclosure: 'Download Server-held Data Disclosure',
    exportingServerDataDisclosure: 'Preparing disclosure...',
    serverDataDisclosureDescription:
      'Web-only, non-importable JSON disclosure of repository connection metadata and the projections, cached attachment bytes, webhook/write history, and RAG index held by MemoFlow. It excludes MemoFlow-managed replayable GitHub authorization, local Vault files, and GitHub history; repository content is included verbatim.',
    createBackup: 'Create Local Backup',
    restoreBackup: 'Restore Backup',
    restoreBackupNoBackups: 'Restore Backup (No backups available)',
    cloudSync: 'Cloud Sync',
    syncAllDevices: 'Sync All Devices',
    syncing: 'Syncing...',
    viewVersionHistory: 'View Version History',
    lastSynced: 'Last Synced',
    version: 'Version',
  },
  resetPreferences: {
    title: 'Reset preferences',
    description:
      'Restore appearance, region, notification and other preferences to their defaults. Resetting a single category leaves every other category untouched.',
    categoryLabel: 'Category',
    categoryAll: 'All categories',
    categoryAppearance: 'Appearance',
    categoryLocale: 'Region',
    categoryNotification: 'Notifications',
    categoryPrivacy: 'Privacy',
    categoryExperimental: 'Experimental',
    resetButton: 'Reset',
    resetting: 'Resetting...',
    currentTheme: 'Current theme',
    currentNotification: 'Notifications enabled',
    themeUnknown: 'Unknown',
  },
  time: {
    justNow: 'Just now',
    minutesAgo: '{n} min ago',
    hoursAgo: '{n} hr ago',
    daysAgo: '{n} days ago',
  },
  errors: {
    loadFailed: 'Failed to load settings',
    updateFailed: 'Failed to update settings',
    resetFailed: 'Failed to reset settings',
    exportFailed: 'Failed to export settings',
    importFailed: 'Failed to import settings',
  },
  userFiles: {
    title: 'File Storage',
    currentDirectory: 'Current Directory',
    defaultDirectory: 'Default Directory',
    customBadge: 'Custom',
    changeDirectory: 'Change Directory',
    openDirectory: 'Open Folder',
    resetToDefault: 'Reset to Default',
    resetConfirm: 'Reset file storage location to default?',
    description: 'Choose where MemoFlow stores your exported files, downloads, and attachments.',
    loadPathFailed: 'Failed to load file storage path',
    directoryChanged: 'File storage location updated',
    pickDirectoryFailed: 'Failed to change directory, please try again',
    openDirectoryFailed: 'Unable to open folder',
    resetSuccess: 'File storage location reset to default',
    resetFailed: 'Failed to reset, please try again',
  },
  tabs: {
    appearance: 'Appearance',
    locale: 'Region',
    editor: 'Editor',
    workflow: 'Workflow',
    repository: 'Repository',
    ai: 'AI',
    privacy: 'Privacy',
    shortcuts: 'Shortcuts',
    notifications: 'Notifications',
    experimental: 'Experimental',
    advanced: 'Advanced',
    userFiles: 'Files',
  },
  editor: {
    editMode: 'Edit Mode',
    defaultOpenMode: 'Default Open Mode',
    defaultOpenModeDescription: 'Default display mode when opening notes',
    reading: 'Read',
    editing: 'Edit',
    readingMode: 'Reading mode',
    editingMode: 'Editing mode',
    autoSaveDelay: 'Auto-Save Delay',
    autoSaveDelayDescription: 'How long after you stop typing to auto-save',
    autoSaveDelayPlaceholder: 'Select delay',
    mediaEmbed: 'Media Embed',
    linkPreview: 'Link Hover Preview',
    linkPreviewDescription: 'Show a preview card when hovering over links',
    enableMediaEmbed: 'Enable Media Embed',
    enableMediaEmbedDescription: 'Display images, audio, and video players directly in notes',
    supportedVideoSites: 'Supported Video Sites',
    supportedVideoSitesDescription:
      'Links from these sites will be automatically converted to embedded players',
    display: 'Display',
    fontSize: 'Font Size',
    fontSizeDescription: 'Text size in the editor',
    showLineNumbers: 'Show Line Numbers',
    showLineNumbersDescription: 'Show line numbers in edit mode',
    showWordCount: 'Show Word Count',
    showWordCountDescription: 'Show the word count of the current note in the status bar',
    mediaEmbedSyntax: 'Media Embed Syntax',
    syntaxEmbedImage: 'Embed image from repository',
    syntaxEmbedAudio: 'Embed audio player',
    syntaxEmbedVideo: 'Embed video player',
    syntaxExternalImage: 'Embed external image',
    syntaxEmbedWeb: 'Embed web video',
    delay300: '300ms (Fast)',
    delay500: '500ms (Recommended)',
    delay1000: '1 second',
    delay2000: '2 seconds',
    delay5000: '5 seconds',
  },
} as const;

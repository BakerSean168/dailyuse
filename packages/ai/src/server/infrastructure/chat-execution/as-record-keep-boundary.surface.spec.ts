...
describe('asRecord/toRecord cross-package keep-boundary (residual 1099)', () => {
  ...
  it('differs from data-portability schedule asRecord {} fallback (no force-merge)', () => {
    ...
    // soft residual may name null; schedule must not use null fallback for asRecord
    expect(scheduleImporter).not.toMatch(/function asRecord\([^)]*\):\s*Record<string, unknown>\s*\|\s*null/);
    expect(scheduleImporter).not.toMatch(/function asRecord[\s\S]{0,200}: null/);
  });

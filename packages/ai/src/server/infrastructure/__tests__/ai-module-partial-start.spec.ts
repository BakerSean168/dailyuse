/**
 * createAIModule partial-start cleanup spec.
 * createAIModule 部分启动回滚测试。
 *
...
interface FakeContribution {
  ...
  readonly contribution: AIModuleRuntimeContribution;
}
...
function makeDeps(runtimeContributions: AIModuleRuntimeContribution[]): AIModuleDependencies {
  return {
...
describe('createAIModule partial-start cleanup', () => {
  it('stops already-started contributions in reverse order and rethrows the original error', () => {
    ...
    const instance = createAIModule(makeDeps([a.contribution, b.contribution, c.contribution]));
  ...
  it('leaves the module unstarted after a failed start: dispose is a no-op', () => {
    ...
    const instance = createAIModule(makeDeps([a.contribution, b.contribution]));

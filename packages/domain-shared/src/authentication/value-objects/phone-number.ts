...
import type { PhoneNumberDTO, PhoneNumberPersistenceDTO, PhoneNumber as IPhoneNumber } from '@dailyuse/contracts/authentication';
...
export class PhoneNumber extends ValueObject<PhoneNumberDTO> implements IPhoneNumber {
  ...
  private static validate(props: PhoneNumberDTO): void {
    ...
    // 中国手机号校验（最常见的格式）
    // 允许格式：13/14/15/16/17/18/19 开头的 11 位数字
  ...
  /**
   * 获取运营商（仅针对中国大陆手机号）
   * @returns 'CHINA_MOBILE' | 'CHINA_UNICOM' | 'CHINA_TELECOM' | 'UNKNOWN'
   */
  public getCarrier(): 'CHINA_MOBILE' | 'CHINA_UNICOM' | 'CHINA_TELECOM' | 'UNKNOWN' {
    ...
    // 中国移动：134-139, 147, 150, 151, 152, 157, 158, 159, 178, 182, 183, 184, 187, 188, 198
    ...
    if (['130', '131', '132', '155', '156', '166', '171', '175', '176', '185', '186', '196'].includes(prefix)) {
      return 'CHINA_UNICOM';
    }

    // 中国电信：133, 149, 153, 173, 177, 180, 181, 189, 190, 191, 193, 199
    if (['133', '149', '153', '173', '177', '180', '181', '189', '190', '191', '193', '199'].includes(prefix)) {

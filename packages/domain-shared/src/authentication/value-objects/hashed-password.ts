...
export class HashedPassword extends ValueObject<HashedPasswordDTO> implements IHashedPassword {
  ...
  public getAgeDescription(): string {
    const days = this.getDaysSinceCreation();
    
    if (days === 0) return '今天刚创建';
   ...
   * 转换为持久化格式（数据库存储）
   * 
   * 将 DTO 中的 number 类型 timestamp 转换为 Date 对象
   * 用于 ORM 映射
   */

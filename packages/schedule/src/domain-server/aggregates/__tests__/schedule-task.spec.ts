...
describe('ScheduleTask Aggregate', () => {
  ...
  describe('fromDTO()', () => {
    it('should create task from a DTO object', () => {
      const taskId = ScheduleTaskId.generate();
      const dto = {
        id: taskId,
        identityId: 'acc-dto',
        name: 'DTO Task',
      ...
      expect(task.name).toBe('DTO Task');
      expect(task.sourceModule).toBe(SourceModule.Goal);
      expect(task.version).toBe(2);
    });

    it('should load executions from DTO when provided', () => {
      const dto = {
        identityId: 'acc-1',
        ...
        metadata: { payload: {}, tags: [], priority: 'Normal', timeout: null },
        createdAt: Date.now(),
        updatedAt: Date.now(),
        version: 1,
        deletedAt: null,
      ...
      expect(task.executions).not.toBeNull();

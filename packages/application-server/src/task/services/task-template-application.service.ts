...
export class TaskTemplateApplicationService {
  ...
  async createTaskTemplate(params: {
    ...
    if (template.status === TaskTemplateStatus.ACTIVE) {
      console.log(
        `[TaskTemplateApplicationService] 模板 "${template.title}" 已创建，开始生成初始实�?..`,
      );
   ...
   * 3. ScheduleTask触发时，检查当天Instance的实际时间，发送提�?
   *
   * 收益�?
   * - 用户体验好（可修改单天时间）
   * - 性能合理（只�?个ScheduleTask�?
  ...
  private async generateInitialInstances(template: TaskTemplate): Promise<void> {
    try {
      ...
      if (instances.length > 0) {
        ...
        // 更新模板�?lastGeneratedDate
        await this.templateRepository.save(template);

        console.log(
          `�?[TaskTemplateApplicationService] 模板 "${template.title}" 生成�?${instances.length} 个实例（未来100天）`,
        );
      ...
      console.error(
        `�?[TaskTemplateApplicationService] 模板 "${template.title}" 初始化失�?`,
        error,
      );
  ...
  private async createScheduleTaskForTemplate(template: TaskTemplate): Promise<void> {
    try {
      ...
      console.log(
        `�?[TaskTemplateApplicationService] 为模�?"${template.title}" 创建了循�?ScheduleTask: ${scheduleTask.uuid}`,
      );
    } catch (error: any) {
      // 如果�?不需要调�?错误，不报错
      if (error?.name === 'SourceEntityNoScheduleRequiredError') {
        console.log(
          `ℹ️  [TaskTemplateApplicationService] 模板 "${template.title}" 不需要创�?ScheduleTask（未配置提醒或不满足条件）`,
        );
        return;
      }

      console.error(
        `�?[TaskTemplateApplicationService] 为模�?"${template.title}" 创建 ScheduleTask 失败:`,
        error,
      );
      // 不抛出错误，ScheduleTask 创建失败不影�?TaskTemplate 创建
    }
  }
  ...
  private async checkAndRefillInstances(template: TaskTemplate): Promise<void> {
    try {
      ...
      if (this.generationService.shouldRefillInstances(template)) {
        console.log(`🔄 [TaskTemplateApplicationService] 模板 "${template.title}" 需要补充实�?..`);
        ...
        if (instances.length > 0) {
          ...
          console.log(
            `�?[TaskTemplateApplicationService] 为模�?"${template.title}" 补充�?${instances.length} 个实例`,
          );
  ...
  async updateTaskTemplate(
    ...
    // TODO: �?TaskTemplate 聚合根中添加 update() 方法
    ...
    if (params.timeConfig || params.recurrenceRule || params.reminderConfig) {
      try {
        ...
        console.log(
          `📤 [TaskTemplateApplicationService] 已发�?task.template.schedule_changed 事件`,
        );
      } catch (error) {
  ...
  async activateTaskTemplate(uuid: string): Promise<TaskTemplateServerDTO> {
    ...
    console.log(`[TaskTemplateApplicationService] 开始激活模�? ${template.title}`);

    // 1. 激活模板状�?
    template.activate();
    await this.templateRepository.save(template);
    console.log(`�?[TaskTemplateApplicationService] 模板状态已更新�?ACTIVE`);

    // 2. 立即生成实例到今�?
    console.log(
      `[TaskTemplateApplicationService] 模板 "${template.title}" 已激活，开始生成实�?..`,
    );
    await this.generateInitialInstances(template);
    ...
    try {
      ...
      console.log(`📤 [TaskTemplateApplicationService] 已发�?task.template.resumed 事件`);
    } catch (error) {
  ...
  async pauseTaskTemplate(uuid: string): Promise<TaskTemplateServerDTO> {
    ...
    console.log(`[TaskTemplateApplicationService] 开始暂停模�? ${template.title}`);

    // 1. 暂停模板状�?
    template.pause();
    await this.templateRepository.save(template);
    console.log(`�?[TaskTemplateApplicationService] 模板状态已更新�?PAUSED`);
    ...
    try {
      ...
      console.log(`📤 [TaskTemplateApplicationService] 已发�?task.template.paused 事件`);
    } catch (error) {
  ...
  private async handleInstancesOnPause(templateUuid: string): Promise<void> {
    try {
      ...
      console.log(`�?[TaskTemplateApplicationService] 已处�?${pendingInstances.length} 个实例`);
    } catch (error) {
  ...
  async deleteTaskTemplate(uuid: string): Promise<void> {
    ...
    try {
      ...
      console.log(`📤 [TaskTemplateApplicationService] 已发�?task.template.deleted 事件`);
    } catch (error) {
  ...
  /**
   * 为模板生成实�?
   * @deprecated 使用新的自动维护机制，不再需要手动指�?toDate
   */
  ...
  async getTaskDashboard(accountUuid: string): Promise<{
    ...
    const [
      ...
      this.getUpcomingTasks(accountUuid, 7), // 未来7�?
      this.getTasksSortedByPriority(accountUuid, 5), // �?个高优先级任�?
      this.getRecentCompletedTasks(accountUuid, 10), // 最�?0个完成的任务

 ...
 *
 * Locks the Step C wiring: apps/api/src/server.ts must compose repository through
 ...
 *
 * 锁定 Step C 接线：apps/api/src/server.ts 必须通过 runtime composer 组装仓库，
...
describe('repository API runtime composer surface', () => {
  ...
  it('server.ts composes repository via composeRepository({ db: prisma, storageBaseDir, closureChecker, githubApp, knowledgeRepositoryCloudDataPurger })', () => {
    expect(server).toContain("from './runtime/compose-repository'");
    ...
    expect(server).toContain('.register(repositoryApiModule)');
  });

  it('server.ts no longer references createRepositoryApiModule or the repository/api seam', () => {
    expect(server).not.toMatch(/\bcreateRepositoryApiModule\b/);
    expect(server).not.toContain("from '@memoflow/repository/api'");
  });

  it('keeps getApplicationPort() as an explicit dependency of composeAI', () => {
    expect(server).toContain('repositoryApiModule.getApplicationPort()');
  });

 ...
 *
 * Locks the Step C wiring: apps/api/src/main.ts must compose setting through the
 ...
 *
 * 锁定 Step C 接线：apps/api/src/main.ts 必须通过 runtime composer 组装设置，
...
describe('setting API runtime composer surface', () => {
  ...
  it('main.ts composes setting via composeSetting({ db: prisma })', () => {
    expect(main).toContain("from './runtime/compose-setting'");
    expect(main).toMatch(/composeSetting\(\{\s*db: prisma,?\s*\}/);
    expect(main).toContain('.register(settingApiModule)');
  });

  it('main.ts no longer references SettingApiModule or the setting/api seam', () => {
    expect(main).not.toMatch(/\bSettingApiModule\b/);
    expect(main).not.toContain("from '@memoflow/setting/api'");
  });

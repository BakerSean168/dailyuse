 ...
 *
 * Locks the Step C wiring: apps/api/src/main.ts must compose data-portability
 ...
 *
 * 锁定 Step C 接线：apps/api/src/main.ts 必须通过 runtime composer 组装
...
describe('data-portability API runtime composer surface', () => {
  ...
  it('main.ts composes data-portability via composeDataPortability({ db: prisma })', () => {
    expect(main).toContain("from './runtime/compose-data-portability'");
    expect(main).toMatch(/composeDataPortability\(\{\s*db: prisma,?\s*\}/);
    expect(main).toContain('.register(dataPortabilityApiModule.module)');
  });

  it('main.ts no longer references DataPortabilityApiModule or the data-portability/api seam', () => {
    expect(main).not.toMatch(/\bDataPortabilityApiModule\b/);
    expect(main).not.toContain("from '@memoflow/data-portability/api'");
  });

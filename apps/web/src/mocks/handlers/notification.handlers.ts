...
export const notificationHandlers = [
  ...
  http.post(BASE, async ({ request }) => {
    ...
    return HttpResponse.json(
      { ok: true, code: 200, message: 'Created', data: createMockNotification(body), timestamp: Date.now() },
      { status: 201 },
    );
  }),
  ...
  http.patch(`${BASE}/read-all`, () => {
    return HttpResponse.json({
      ok: true, code: 200, message: 'All marked as read',
      data: { count: 5 },
      timestamp: Date.now(),
    });
  }),
  ...
  http.post(`${BASE}/batch-delete`, () => {
    return HttpResponse.json({
      ok: true, code: 200, message: 'Batch deleted',
      data: { count: 0 },
      timestamp: Date.now(),
    });
  }),
  ...
  http.patch(`${BASE}/:id/read`, ({ params }) => {
    return HttpResponse.json({
      ok: true, code: 200, message: 'Marked as read',

...
function formatRangeText(range: TimeRange): string {
  ...
  switch (type) {
    case 'day':
      return format(start, 'yyyyå¹´Mæœˆdæ—?, { locale: zhCN });
    case 'week':
      return `${format(start, 'Mæœˆdæ—?, { locale: zhCN })} - ${format(end, 'Mæœˆdæ—?, { locale: zhCN })}`;
    case 'month':
      return format(start, 'yyyyå¹´Mæœ?, { locale: zhCN });
    case 'quarter':
      const quarter = Math.floor(start.getMonth() / 3) + 1;
      return `${start.getFullYear()}å¹´Q${quarter}`;
    case 'year':

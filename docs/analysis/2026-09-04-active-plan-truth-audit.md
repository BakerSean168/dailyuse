...
6. Production remains on the healthy operator-restored old exact runtime with the v0.13.2 `BLOCKED` receipt retained and timer disabled. Recovery review proved `/opt/memoflow/runtime.prev` matches all six current live service refs, while `/opt/memoflow/runtime` is the failed target root. The next control-plane repair requires `BLOCKED + --force` to verify that preserved live baseline before rotation, then re-select the same Published v0.13.3 under the new `controlPlaneSha` and complete force -> new backup -> exact state -> replay -> timer enable.
...
**Recommendation:** do not start another delivery redesign and do not create v0.13.4 for a control-plane-only recovery fix. v0.13.3 is already Published and selected with the corrected Migrator. Merge the focused BLOCKED live-baseline guard, re-run `Deploy Production(v0.13.3)` so the new `controlPlaneSha` is bound into the desired set, then perform coherent check-only -> deliberate `--force` recovery with a new backup -> exact state -> idempotent replay -> timer enable.

...
tags:
  ...
  - deployment
  - desktop
description: MemoFlow Delivery Platform V3 已完成实施、生产切换与 Phase 4 收口的归档计划
...
> **Archived 2026-09-06:** Phase 1–4 are complete. `v0.13.3` remains the current Published production release and Alibaba production is watcher-owned at exact `DEPLOYED` state. DLV3-4401 retained split quality runners from five paired samples; DLV3-4402 completed repository-wide immutable Action pinning; DLV3-4403 implemented fail-closed app+DMG signed/notarized trust while the current product policy remains explicitly `unsigned-pilot`; DLV3-4404 closed final CI/coverage observation and repository truth. Trusted-public macOS activation remains a future Apple-credential gate, not an active Delivery V3 task.
...
| DLV3-1104      | **COMPLETE for Phase 1 scope** | touched release workflows use pinned reviewed Actions; repository-wide convergence is now closed by DLV3-4402.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Phase 4        | **COMPLETE — 2026-09-06**      | DLV3-4401 retained split after five real paired samples; DLV3-4402 completed repository-wide immutable Action pinning; DLV3-4403 implemented fail-closed app+DMG Developer ID/notary/stapler/Gatekeeper trust while `MACOS_RELEASE_MODE=unsigned-pilot` remains the explicit product policy; DLV3-4404 closed observation after #328/#329, exact-main CI and Coverage both passed on `cc612e83...`, and this plan moved to archive. Trusted-public macOS activation remains a future conditional gate requiring Apple credentials and a real signed-release acceptance.                                                                                                                                                                                                                                                                                                                                              |
...
Delivery Platform V3 is now **closed and archived**. Phase 3 production acceptance remains immutable at `v0.13.3 -> 4e24cffd64b2255a188d28dbe37308a1e2a3fe3a` with control-plane `64521cf122753a23c0d954b2a7c46a65b93ba028`, production-set `sha256:2ea3a1127ffc179e4f24deb1d518246a7f9522bbe4a2801e7ed6c8647f1ae0ae`, control artifact `sha256:4d9b493f41c330dd31fc6ecb183d8cc165851ab38cd226ed6da4daf0943175fa` and exact Migrator `sha256:e734a0ebaa82479413ef2e8bd7641ae781ddb09577a9be36f6f9ac4d7d517ad3`. Alibaba state is `DEPLOYED`; replay is idempotent; independent GCP public ingress is healthy; the production watcher timer is enabled/active. Phase 4 closed the runner-efficiency decision (DLV3-4401: retain split), repository-wide third-party Action pinning (DLV3-4402), fail-closed macOS signed/notarized capability (DLV3-4403), and final CI/Coverage observation plus documentation/archive (DLV3-4404). `MACOS_RELEASE_MODE=unsigned-pilot` is explicitly configured; Apple signing/notarization credentials remain absent, so trusted-public macOS activation is intentionally deferred.
...
**Implementation status:** COMPLETE — **RETAIN SPLIT**.

The experiment kept required CI unchanged and used the manual-only `.github/workflows/quality-runner-shadow.yml` on canonical main `55f2825196008030ef390178e98cf6c6393168d6`. Five paired runs (`34008548967`, `34008551247`, `34008553090`, `34008555415`, `34008557924`) all executed the same current Static / Unit / Typecheck / Build command strings against the fixed Desktop-heavy PR #286 affected range (`ee8472486377c30ecced3ef1ec07012fd8a06f24 -> ca3970a4c536addfc48409db1c22ebb4e0ce9ae6`). Every logical child passed in both profiles and every paired aggregate passed command/workload/control-plane/topology parity.

The versioned evidence is committed under `reports/ci-cd-platform/quality-shadow/2026-09-06/`; `report-v1.json` is reproducible from the five self-digested samples and has digest `7286921ed867531e478dbc0f93bac5718bd5e5f9c2ff4c40634fcde1515f84e9`.

| Metric                |         Split P50 / P95 |  Consolidated P50 / P95 | Decision signal                                       |
| --------------------- | ----------------------: | ----------------------: | ----------------------------------------------------- |
| setup                 |     104.257s / 105.088s |       29.087s / 32.810s | consolidated saves repeated setup                     |
| logical execution sum |     523.156s / 529.908s |     421.338s / 436.225s | consolidated benefits from one warm workspace         |
| longest child         |     215.417s / 227.895s |     223.463s / 233.140s | slight consolidated regression                        |
| wall clock            | **247.706s / 266.173s** | **458.639s / 473.372s** | consolidated regresses by 210.933s P50 / 207.200s P95 |
| runner-minutes        |     **11.108 / 11.207** |       **7.644 / 7.890** | consolidated saves 3.464m P50 / 3.318m P95            |

The predeclared promotion gate required **both** non-regressing P50/P95 wall time and improved P50/P95 runner-minutes. Runner cost improved, but wall time regressed strongly in both percentiles; therefore the deterministic comparator returned `retain-split`. Required Static / Unit / Typecheck / Build jobs remain physically split. The manual shadow workflow and comparator remain available for future re-measurement if runner setup costs or GitHub-hosted runner characteristics change.

### DLV3-4402 — Pin every third-party Action

**Implementation status:** COMPLETE — all third-party `uses:` refs under `.github/workflows/**` and `.github/actions/**` are immutable 40-hex commit pins with inline version comments. The repository-wide governance audit rejects mutable tags/branches, abbreviated SHAs, undocumented pins and unsupported non-repository third-party `uses:` syntax while exempting repository-owned local actions.

The convergence pinned 45 previously mutable occurrences across nine tag families without changing their major versions: `actions/checkout@v6`, `actions/setup-node@v6`, `actions/setup-python@v6`, `astral-sh/setup-uv@v7`, `actions/cache@v5`, `actions/upload-artifact@v6`, `actions/download-artifact@v7`, `docker/login-action@v3` and `actions/github-script@v8`. Each tag was resolved from its upstream Git ref before replacement; no SHA was guessed.

**Acceptance:** `github-action-pinning-audit` scans every workflow/composite Action file and fails closed on mutable or undocumented third-party refs; positive/negative governance fixtures cover immutable pins, local actions, tags/branches/abbreviated SHAs, missing version comments and unsupported third-party syntax. Test inventory includes the new governance suite.

### DLV3-4403 — macOS signing/notarization

**Implementation status:** CODE COMPLETE — trusted-public activation remains credential-gated; current release policy stays **unsigned-pilot**.

The release lane now has one explicit macOS policy input: `unsigned-pilot` or `signed-notarized`. Release Publish resolves it from repository variable `MACOS_RELEASE_MODE` and defaults to `unsigned-pilot`; direct Release Assets recovery dispatch exposes the same bounded choice. There is no silent signed → unsigned fallback.

Signed mode requires protected Developer ID + App Store Connect credentials before packaging: `MACOS_CSC_LINK`, `MACOS_APPLE_API_KEY_P8`, `MACOS_APPLE_API_KEY_ID`, `MACOS_APPLE_API_ISSUER` (plus `MACOS_CSC_KEY_PASSWORD` when the certificate bundle requires one). The credential preflight validates the App Store Connect key shape, materializes the `.p8` only in runner temp with mode 0600 and deletes it with an `always()` cleanup.

The signed electron-builder overlay forces Developer ID distribution signing and hardened runtime. electron-builder signs/notarizes/staples the `.app`; the DMG target is separately forced to Developer ID signing, then MemoFlow submits the final DMG to `notarytool --wait`, requires `Accepted`, staples the ticket and validates it. DMG update metadata is disabled before this post-build mutation so no blockmap can describe pre-staple bytes.

A versioned `macos-trust-receipt/v1` is emitted only after an independent read-only verifier proves **both** the application and disk image: matching Developer ID authority, application TeamIdentifier + hardened runtime, stapled tickets, Gatekeeper `Notarized Developer ID` acceptance, and the expected x64/arm64 executable architecture. `signed-notarized` platform receipts and the canonical Desktop manifest fail closed if this trust evidence is absent, forged or identity-mismatched. Unsigned pilot receipts cannot carry signed trust evidence.

As of 2026-09-06 the GitHub repository variable is explicitly `MACOS_RELEASE_MODE=unsigned-pilot`, while the repository/environments contain no Apple Developer signing/notarization credentials. Therefore no real signed/notarized MemoFlow release has been accepted and current macOS assets remain explicit internal/manual `unsigned-pilot` artifacts. This is a **conditional product activation gate**, not a blocker for the already-accepted Server production transaction or this plan archive. To enable trusted public macOS distribution, provision the protected Apple credentials, change `MACOS_RELEASE_MODE=signed-notarized`, and require both macOS release lanes to produce valid app+DMG trust receipts before publication.

### DLV3-4404 — Close observation and archive

**Implementation status:** COMPLETE — 2026-09-06.

Final observation closed the remaining platform evidence without reopening the accepted v0.13.3 production transaction:

- PR #328 (`a8e6108521e66292280f5744815284830a83c866`) implemented DLV3-4403 and passed full PR CI run `34010162847`; merge `706c432cb4fb5bba83eaac5f07d869c3bd83095c` then passed full main CI `34010689498`.
- Coverage run `34010689541` exposed a low-frequency control-plane race before any coverage target ran: concurrent scope discovery spawned multiple `pnpm exec nx show projects` processes and one dependency-status wrapper failed. #329 replaced that wrapper with the lockfile-owned Nx Node CLI while preserving parallel scope queries; six local full-scope stress runs produced the same manifest digest.
- PR #329 (`a6e14c52fa59e0d74223c3b733bd0bce1caa746f`) passed full PR CI `34011550696`; merge `cc612e83c5781f6e09dfe63a34764eb5d67fed6d` passed full main CI `34012124207` **19/19** and Coverage `34012124229` end-to-end, including manifest generation, target detection, governed coverage gates and evidence publication.
- Candidate run `34012669026` succeeded for the exact final main SHA. Prepare Release updated the existing `0.14.0` Release PR #322, while Release Publish `34012838428` correctly no-op'd because no release PR was merged. No new Draft/Published release or production selection was created; v0.13.3 remains production authority.
- `reports/ci-cd-platform/quality-shadow/2026-09-06/report-v1.json` remains the final timing baseline/decision evidence; deployment SSOT already records the accepted v0.13.3 exact production state and idempotent watcher replay.
- Repository policy is explicitly `MACOS_RELEASE_MODE=unsigned-pilot`; Apple credentials are absent. Trusted-public macOS remains a future conditional activation and is not represented as accepted evidence here.

With staging proof, Published release proof, production proof, performance evidence, supply-chain hardening, macOS trust capability and final CI/Coverage observation all recorded, this plan is archived.

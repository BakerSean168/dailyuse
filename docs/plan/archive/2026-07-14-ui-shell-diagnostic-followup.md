...
> **决策修订（2026-08-06）**：本文关于“统一 launcher、移除常驻胶囊”、固定业务最大宽度和设置场景栏的描述已由 [基础 UI 与 Shell 重构](../active/2026-08-06-ui-foundation-and-shell-refactor.md) 修订。当前实现恢复复合胶囊与 Schedule/Notification 入口，移除产品级固定最大宽度，并将设置页扁平化为页头 + 分类导航 + 单一滚动容器；本文其余历史诊断记录仍保留供追溯。

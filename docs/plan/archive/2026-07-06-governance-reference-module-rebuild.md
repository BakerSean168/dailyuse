...
- 公共契约唯一真值源固定为 `@memoflow/contracts/governance`
- 运行时根入口只承担服务端组合根职责
...
- mocks 收敛到 `@memoflow/contracts/mocks`

## 最终公开结构

```text
@memoflow/contracts/governance  # 公共契约
@memoflow/contracts/mocks       # governance mock 也在这里

@memoflow/governance            # 服务端组合根
...
3. 迁移 Web、Desktop renderer、Desktop preload tests 到新的 `@memoflow/governance/client`。
4. 移除 app-vue 对 `@memoflow/governance/domain-client` 的依赖，把展示派生和 hydration 迁到 `packages/app-vue/src/modules/governance/`。
...
8. 运行最小验证，至少覆盖 `contracts`、`governance`、desktop/web 调用方与 `memoflow:governance-check`。
...
- `packages/app-vue` 不再引用 `@memoflow/governance/domain-client`
- governance mocks 只从 `@memoflow/contracts/mocks` 导入

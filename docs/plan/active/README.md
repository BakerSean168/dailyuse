...
| [GitHub App + MagicDNS 产品闭环与复审问题收敛](./2026-08-25-github-app-magicdns-product-hardening.md)                       | **In progress**：OAuth/App runtime + live product hardening；installation orchestration 由 ADR-065 子计划接管                                                                              |
| [GitHub Durable Installation Intent + Setup Gateway](./2026-08-28-github-installation-intent-gateway.md)                    | **Implementation / verification**：durable intent → public callback-only gateway → authenticated finalize → Web/Desktop completion → dev/staging routing → Docker/staging gate             |

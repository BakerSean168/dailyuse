...
将 `packages/governance/src/contracts` 中的公共数据契约彻底迁移到 `packages/contracts/src/modules/governance`，移除 `@dailyuse/governance/contracts` 子路径与本地 contracts 目录，使 governance 与其他 feature module 的 contracts 组织完全一致。
...
4. 将 governance 包内所有对本地 `contracts` 的依赖改为 `@dailyuse/contracts/governance` 或 feature-local port。
...
8. 运行 `contracts`、`governance`、调用方的定向验证，并补跑 `daily-use:governance-check`。

## 完成标准

- `packages/governance/src/contracts` 不再存在
- 外部调用统一通过 `@dailyuse/contracts/governance`

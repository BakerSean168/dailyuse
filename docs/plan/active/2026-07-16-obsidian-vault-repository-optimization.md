...
> Host task.create 进程内产品路径（残留 427–463）：AgentType + toolMode + start/store/resume(cancel/confirm/edit)
> + history/session focus + linked goal restore + client settlement 会话隔离 + store 容量边界 + process-local product journey + runId 身份/会话/thread 绑定 + confirm 必须 client executedActions + edit 非空 title + dirty approve 先 process-local revise + start 必须 conversationId + confirm settlement title 可回收；永不 `executeApproved`。

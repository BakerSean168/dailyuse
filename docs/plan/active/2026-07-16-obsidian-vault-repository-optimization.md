...
> 审计时间 2026-07-22（残留一百九十一轮刷新证据指针）。状态标记：已证明 / 部分实现 / 外部阻塞 / 仍未实现。只有证据充分才改 checkbox。
> 身份隔离 dual-method 收口（残留 169–179）：业务聚合 bare `findById` 双轨已基本拆除，仅保留
> 已 surface 锁定的 intentional dual/bootstrap：schedule-task（180）、knowledge connection（186）、
> webhook delivery（187）、auth session token（188）、notification template catalog（189）、
> governance rule catalog（190）；另 account/auth identity 主键路径仍为自然主键。

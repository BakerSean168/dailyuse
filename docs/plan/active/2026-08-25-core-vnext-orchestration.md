...
- the shared `LabelFilterPopover` is now wired end-to-end through `CanonicalTaskTemplateListQuery.labelIdsAll` and `TaskClientPort.listTemplates(...)`. The server-filtered template projection is treated as the authority for Label AND and Goal filters, so the occurrence surface does not post-filter one arbitrary paginated page or reimplement label semantics. Task-lane closeout also added the missing Express query-parser pass-through plus route regression, proving `labelIdsAll` reaches the controller/use-case on the Web HTTP path rather than existing only in contracts/client state;

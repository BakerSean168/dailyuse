...
| Phase 4        | **IN PROGRESS — 4401 SHADOW + 4402 COMPLETE** | DLV3-4402 repository-wide Action pinning is complete. DLV3-4401 now has a manual-only paired split-vs-consolidated quality-runner shadow harness with immutable child receipts and standard P50/P95 comparison; five real comparable samples and a promotion/retain decision remain. macOS signing/notarization and final observation/archive also remain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
**Implementation status:** SHADOW HARNESS IMPLEMENTED; real sample collection pending.

The experiment keeps required CI unchanged. `.github/workflows/quality-runner-shadow.yml` is manual-only and runs one paired benchmark against the same current control-plane revision and the fixed Desktop-heavy PR #286 affected range (`ee847248... -> ca3970a4...`). One side executes Static / Unit / Typecheck / Build as four physical jobs; the other executes the exact same four command strings sequentially on one physical runner after one workspace setup. Each logical child emits a self-digested receipt with workload, control-plane SHA, command, setup/execution timestamps and status; the paired sample rejects command/workload/control-plane drift, missing children, hidden failures or incorrect 4:1 physical topology.

`compare-timings.mjs` now exposes the same standard P50/P95 timing comparator for raw comparable timing samples. `compare-quality-shadow.mjs` requires at least five successful unique paired samples with one workload signature and evaluates setup time, execution time, longest child, wall-clock time and runner-minutes. Consolidated execution is promoted only when P50 and P95 wall time do not regress **and** P50/P95 runner-minutes improve; otherwise the evidence-based decision is `retain-split`. A single fast run can never promote the topology.

**Acceptance still pending:** merge the shadow harness, dispatch at least five real `desktop-pr286` paired runs from the default branch, preserve all sample artifacts, produce the timing report and either (a) promote the consolidated physical runner in a separate reviewed PR while keeping Validate Oracle and logical child evidence, or (b) close DLV3-4401 with a measured `retain-split` decision.

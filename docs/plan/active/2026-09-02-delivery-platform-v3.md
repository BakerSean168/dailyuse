...
> Status: ACTIVE — Phase 1 + Phase 2 closed; Phase 3 release + selector accepted; v0.13.3 is Published and re-selected with BLOCKED rollback-baseline hardening; Alibaba remains on the recovered old exact runtime while host-local Caddy transaction probing replaces unreliable public hairpin probing
...
| Phase 3        | **IN PROGRESS / HOST PROBE HARDENING** | #310 + #311 + #319 + #320 + #321 established build-once Release, production selector/watcher, Linux release-tooling authority, offline Prisma engine closure and safe BLOCKED recovery baseline validation. `v0.13.3 -> 4e24cffd...` is Published; Migrator `e734a0eb...17ad3` passes offline engine resolution on Alibaba. After #321 merged, selector `33978120212` re-selected the same release under control-plane `3adafc21...` as production-set `sha256:50163b7b...ba1bb` / control artifact `sha256:65a0c04a...a2a22`; the exact bundle was installed without enabling the timer, and host `--check-only` returned `PRODUCTION_SELECTION=COHERENT` while retaining the v0.13.2 `BLOCKED` receipt. Immediately before force, `runtime.prev` again matched all six live service refs. The final preflight then proved Alibaba cannot reliably hairpin its own public HTTPS/DNS routes, while the same canonical HTTPS Host/SNI routes pass 3/3 through local Caddy with `curl --resolve ...:127.0.0.1`. Do not cross the migration boundary until the watcher uses host-local Caddy transaction probes; public reachability remains an independent GCP acceptance check. |
...
The next boundary is **Phase 3 host-probe hardening, then explicit BLOCKED recovery acceptance**. `v0.13.3` remains Published and immutable at `4e24cffd64b2255a188d28dbe37308a1e2a3fe3a`; the corrected Migrator digest `sha256:e734a0ebaa82479413ef2e8bd7641ae781ddb09577a9be36f6f9ac4d7d517ad3` is offline-self-contained and the same release does not need to change. #321 merged the BLOCKED rollback-baseline guard as control-plane `3adafc21ea60ae363468c3dac3da13b6db65d99a`; full main CI passed, selector `33978120212` re-selected v0.13.3 as production-set `sha256:50163b7b77910c2666e002c84272236649723fdb05d7cb575d5ee9e17f4ba1bb` with control artifact `sha256:65a0c04af82b11824611b882493a310e4a51e1707564980cd7840349b19a2a22`, and that exact bundle was installed on Alibaba with the timer still disabled. `--check-only` returned coherent while managed state stayed the retained v0.13.2 `BLOCKED` receipt, and `runtime.prev` still matches live API/Web/PowerSync/Postgres/Redis/Caddy 6/6. A final no-write preflight found Alibaba cannot reliably resolve/hairpin the three public production URLs from the host itself, so the existing final `curl` step would create a false post-migration BLOCKED outcome. The canonical HTTPS routes all pass 200 when forced through local Caddy with `--resolve <host>:443:127.0.0.1`. Merge the host-local Caddy probe repair, re-select the same Published v0.13.3 under that new `controlPlaneSha`, install/check-only the selected bundle, then run explicit `--force` with a new mandatory PostgreSQL backup. After exact state commit and idempotent replay, verify public HTTPS independently from GCP and only then enable the timer.
...
**Implementation status:** IN PROGRESS / REAL FAILURE + RECOVERY SEMANTICS PROVEN — install + coherent check-only + mandatory backup + post-migration `BLOCKED` were exercised on Alibaba; BLOCKED force recovery now requires a preserved live-matching rollback root. Final host health probing is being corrected to validate canonical HTTPS routes through local Caddy rather than unreliable Alibaba public hairpin/DNS. Successful v0.13.3 rollout/replay/public-GCP probe/timer enable remain.

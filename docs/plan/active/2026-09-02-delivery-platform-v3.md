...
> Status: ACTIVE — Phase 1 + Phase 2 closed; Phase 3 control plane implemented; v0.13.2 app/runtime regression fixed, Linux headless Secret Service release-tooling retry + Alibaba acceptance pending
...
| Phase 3        | **IN PROGRESS / RELEASE FAIL-CLOSED** | #310 + #311 merged DLV3-3301~3304. `v0.13.2` is now an immutable Draft at `08c2daf16e145d228d7d7a20c3282486dca58b0d`; Server candidate promotion succeeded and packaged runtime smoke passed on Windows x64, macOS x64 and macOS arm64, proving the ResizeObserver fatal-classification fix on the previously failing arm64 lane. Linux never reached Playwright because pristine GitHub runners activated `org.gnome.keyring.SystemPrompter` during headless Secret Service initialization and the outer Xvfb/D-Bus session was not bounded. Production remains untouched; retry the same Draft with repaired release tooling before selection. |
...
The next boundary is **Phase 3 corrective release acceptance**. `v0.13.0` and `v0.13.1` remain immutable failed Draft releases. `v0.13.2` is also immutable at release source `08c2daf16e145d228d7d7a20c3282486dca58b0d`, but its application-level ResizeObserver fix is proven on Windows x64, macOS x64 and macOS arm64. Repair only the Linux headless Secret Service release tooling, then use the existing `workflow_dispatch(tag)` retry path to resume/finalize the same Draft `v0.13.2`; do not mint `v0.13.3` merely for packaging-tooling changes. Only after all four packaged Desktop runtime gates pass and `v0.13.2` is Published may `Deploy Production(v0.13.2)` select it and the Alibaba watcher proceed through check-only, mandatory backup, controlled rollout, exact-digest state and idempotent replay.
...
- the run was cancelled fail-closed after the outer-session defect was proven. `v0.13.2` remains Draft, production is untouched, and the correct recovery is to harden release tooling and retry the same immutable Draft via `workflow_dispatch(tag=v0.13.2)`.
